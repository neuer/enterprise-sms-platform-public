...
const compactViews = ["OpsView.vue"].map(
...
describe("全站筛选布局契约", () => {
  ...
  it("上行回复使用方案 A 单行检索条", () => {
    ...
    expect(css).toMatch(/\.reply-filter-bar,\s*\.blacklist-filter-bar,\s*\.sensitive-filter-bar,\s*\.user-filter-bar,\s*\.config-filter-bar,\s*\.callback-filter-bar\s*\{[^}]*display:\s*flex/s)
    expect(css).toMatch(/\.reply-seg,\s*\.blacklist-seg,\s*\.sensitive-policy-seg,\s*\.user-seg,\s*\.config-seg,\s*\.callback-seg\s*\{[^}]*border-radius:\s*7px/s)
  })
  ...
  it("用户与角色使用方案 A 单行检索条与身份台账", () => {
    ...
    expect(css).toMatch(/\.reply-filter-bar,\s*\.blacklist-filter-bar,\s*\.sensitive-filter-bar,\s*\.user-filter-bar,\s*\.config-filter-bar,\s*\.callback-filter-bar\s*\{[^}]*display:\s*flex/s)
    expect(css).toMatch(/\.reply-seg,\s*\.blacklist-seg,\s*\.sensitive-policy-seg,\s*\.user-seg,\s*\.config-seg,\s*\.callback-seg\s*\{[^}]*border-radius:\s*7px/s)
    expect(css).toMatch(/\.user-rules,\s*\.config-rules,\s*\.callback-rules\s*\{[^}]*display:\s*flex/s)
  })
