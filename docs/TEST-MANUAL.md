...
| 号码搜索 | HMAC 精确查询、mask、授权解密、时间线、无结果与越权 |
...
| 回调任务 | 状态筛选、dead、详情、手动重推、次数/时间和错误摘要 |
...
安全日报配置验收：管理员在 `/security-daily` 页面打开“配置邮件”，确认可以保存启停状态、Resend API Key 和 1–3 个收件人；页面只显示 Key 是否已配置。保存后启动独立 mailer，使用一条已生成日报执行“安全预览 → 手动投递”，确认状态能够回写为已投递或投递失败。Key 不写入普通系统参数页，邮件正文仅来自脱敏结构化报告。
