"""子批次关系必须证明内部创建；存量保守保留待核验。"""

from __future__ import annotations

from alembic import op

revision = "0110_uncertain_child_provenance"
down_revision = "0109_report_timeout_fairness"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    """)
    op.execute("""
        CREATE OR REPLACE VIEW uncertain_resend_creation_evidence AS
    ...
    """)
    op.execute("GRANT SELECT ON uncertain_resend_creation_evidence TO sms_send")
    op.execute("GRANT SELECT, INSERT, UPDATE ON idempotency_claim TO sms_send")
    op.execute("GRANT INSERT ON idempotency_record TO sms_send")
    op.execute(
        "GRANT USAGE, SELECT ON SEQUENCE idempotency_claim_id_seq, idempotency_record_id_seq "
        "TO sms_send"
    )
    # 关系表沿用既有权限；审计证据只暴露内部主体与批次引用，不升级未知历史关系。
...
"""
    ...
    op.execute("REVOKE SELECT, INSERT, UPDATE ON idempotency_claim FROM sms_send")
    op.execute("REVOKE INSERT ON idempotency_record FROM sms_send")
    op.execute(
        "REVOKE USAGE, SELECT ON SEQUENCE idempotency_claim_id_seq, idempotency_record_id_seq "
        "FROM sms_send"
    )
    op.execute("DROP VIEW uncertain_resend_creation_evidence")
    op.execute("ALTER TABLE sms_uncertain_child DROP COLUMN provenance_verified")
