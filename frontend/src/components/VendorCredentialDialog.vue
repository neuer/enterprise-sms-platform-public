...
const secureContextAvailable = isVendorCredentialSecureContext()
...
async function submit(): Promise<void> {
  ...
  if (!secureContextAvailable) {
    ElMessage.error("当前入口不支持正式凭据安全加密，请通过临时 HTTPS 安全入口重新登录")
    return
  }
...
<template>
  <el-dialog
    ...
    @closed="clear"
  >
    <div v-if="modelValue && secureContextAvailable" class="vendor-sensitive-form">
      <el-alert
