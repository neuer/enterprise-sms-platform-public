...
4. 独立窗口执行账号失败阈值恢复/IP 限流 UAT-02～UAT-03；
...
| SMK-05a | 退出后 Storage 删除失败 | 浏览器允许读取 Storage 但 `removeItem` 抛错时登录再退出 | 本页不再附带旧 Bearer；刷新只依赖 HttpOnly Refresh 或重新登录，不得从残留 `sms_token` 恢复旧 Access Token |
...
### UAT-02 账号连续失败标记与正确凭据恢复

- **优先级：** P0
- **测试角色：** operator01；admin01 备用
- **前置条件：** 两个独立源 IP、无其他人使用 operator01；测试负责人批准失败阈值验证。
- **测试数据：** operator01、错误密码。
- **步骤：**

1. 连续 5 次提交错误密码；
2. 确认第 5 次错误触发 `ACCOUNT_LOCKED` 后，从第二个未限流 IP 使用正确密码登录；
3. 再提交一次错误密码，确认成功登录已清除原账号失败状态；
4. 记录错误码、时间和两个来源 IP。

- **预期结果：** 第 1–4 次错误返回 401，第 5 次错误返回 HTTP 423、`ACCOUNT_LOCKED`；第二个 IP 的正确凭据仍可登录并清除失败标记，随后一次错误重新按第 1 次失败返回 401；错误不泄露 LDAP/Redis 细节。原 IP 仍受独立 prehash/IP 限流约束。
- **证据：** 第 5 次失败、第二 IP 正确登录、清除后首次错误的响应截图。
- **恢复：** 确认 operator01 可正常登录并退出两个窗口。
...
- **预期结果：** API 内心跳巡检发现 stalled 并告警；恢复后任务格转绿；手动触发成功且审计；beat 单实例，第二实例抢锁失败退出。单一 reconcile 域持续失败时，其余恢复域本轮仍应执行，并出现 `reconcile_domain_failed` 告警；总任务可表现为部分失败，不得静默吞错。
