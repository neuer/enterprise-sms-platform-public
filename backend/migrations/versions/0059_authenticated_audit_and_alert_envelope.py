"""Authenticate audit context and isolate the alert credential decryptor."""

from __future__ import annotations

from alembic import op

revision = "0059_authenticated_audit_alert"
down_revision = "0058_audit_writer_enforcement"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS audit_context_signing_key (
          key_kind text PRIMARY KEY CHECK (key_kind IN ('principal','system')),
          key_material bytea NOT NULL CHECK (octet_length(key_material)=32),
          updated_at timestamptz NOT NULL DEFAULT now()
        )
        """
    )
    op.execute("REVOKE ALL ON audit_context_signing_key FROM PUBLIC")
    op.execute(
        r"""
...
BEGIN
  ...
  IF NEW.actor_subject_kind='system' THEN
    ...
    IF session_user<>'sms_owner' THEN
      SELECT key_material INTO audit_key
      FROM public.audit_context_signing_key WHERE key_kind='system';
      IF audit_key IS NULL THEN
    ...
    system_allowed := session_user='sms_owner'
      ...
      OR (
        ...
        AND (
          ...
          OR (
            NEW.actor='import-parser'
            AND NEW.action='message_import'
          )
          OR (
...
$$
    ...
    op.execute(
        "UPDATE sys_config SET value='',updated_at=now() "
        "WHERE key='alert_wecom_webhook' AND btrim(value)<>''"
    )
    op.execute("ALTER TABLE sys_config DROP CONSTRAINT IF EXISTS ck_sys_config_wecom_ciphertext")
    op.execute(
        """
...
def downgrade() -> None:
    # fail closed：不恢复 caller-writable GUC 信任或历史可解密信封。
    pass
