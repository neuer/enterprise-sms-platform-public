...
<template>
  ...
  >
    <header class="ops-panel-title ops-alert-title">
      ...
      <div class="ops-seg" role="group" aria-label="告警视图" data-testid="ops-alert-mode">
        <button type="button" :class="{ on: alertMode === 'current' }" @click="setAlertMode('current')"
          >当前告警</button
        >
        <button type="button" :class="{ on: alertMode === 'history' }" @click="setAlertMode('history')"
          >告警历史</button
        >
      </div>
    </header>
    ...
    <template v-else>
      ...
      <section class="ops-results">
        ...
        <footer class="ops-pagination"
          ><span>共 {{ alertTotal }} 条 · 每页 20</span
          ><el-pagination
            v-model:current-page="alertPage"
            data-testid="ops-alert-pagination"
            :page-size="DEFAULT_PAGE_SIZE"
            :total="alertTotal"
            layout="prev, pager, next"
            @current-change="load('alerts')"
        /></footer>
      </section>
    </template>
  </section>
  ...
  >
    ...
    <section class="ops-results">
      ...
      /></div>
      <footer class="ops-pagination"
        ><span>共 {{ rawTotal }} 条 · 每页 20</span
        ><el-pagination
          v-model:current-page="rawPage"
          data-testid="ops-raw-pagination"
          :page-size="DEFAULT_PAGE_SIZE"
          :total="rawTotal"
          layout="prev, pager, next"
          @current-change="load('raw')"
      /></footer>
    </section>
  </section>
      ...
      <div class="ops-mobile-list"
        ><article v-for="item in uncertain" :key="item.chunk_id"
          ...
          ><el-button
            ...
            >确认处置</el-button
          ></article
        ><EmptyState v-if="!uncertain.length" :title="UNCERTAIN_EMPTY.title" :description="UNCERTAIN_EMPTY.description"
      /></div>
      <footer class="ops-pagination"
        ><span>共 {{ uncertainTotal }} 项 · 每页 20</span
        ><el-pagination
          v-model:current-page="uncertainPage"
          data-testid="ops-uncertain-pagination"
          :page-size="DEFAULT_PAGE_SIZE"
          :total="uncertainTotal"
          layout="prev, pager, next"
          @current-change="load('uncertain')"
      /></footer>
    </section>
  </section>
  ...
  >
    <header class="ops-panel-title"
      ><div><strong>迁移期无主报告</strong><small>无法匹配平台批次的状态报告在此留存，仅供对账核查</small></div></header
    >
    ...
    <section class="ops-results">
      ...
      /></div>
      <footer class="ops-pagination"
        ><span>共 {{ unmatchedTotal }} 条 · 每页 20</span
        ><el-pagination
          v-model:current-page="unmatchedPage"
          data-testid="ops-unmatched-pagination"
          :page-size="DEFAULT_PAGE_SIZE"
          :total="unmatchedTotal"
          layout="prev, pager, next"
          @current-change="load('unmatched')"
      /></footer>
    </section>
  </section>
    ...
    <form class="ops-filter-bar" @submit.prevent>
      <div class="ops-fld"
        ...
        <div class="ops-seg" role="group" aria-label="Outbox 事件状态筛选" data-testid="ops-outbox-state">
          <button
            type="button"
            :class="{ on: outboxState === '' }"
            data-testid="ops-outbox-state-all"
            @click="setOutboxState('')"
            >全部</button
          >
          <button
    ...
    <section class="ops-results">
      ...
      <div class="ops-mobile-list"
        ><article v-for="item in outboxEvents" :key="item.id"
          ...
          ><p
            >{{ shortTaskName(item.task_name) }} · {{ item.queue }} · 尝试 {{ item.attempts }}/{{ item.max_attempts }} ·
      ...
      /></div>
      <footer class="ops-pagination"
        ><span>共 {{ outboxTotal }} 条 · 每页 20</span
        ><el-pagination
          v-model:current-page="outboxPage"
          data-testid="ops-outbox-pagination"
          :page-size="DEFAULT_PAGE_SIZE"
          :total="outboxTotal"
          layout="prev, pager, next"
          @current-change="load('outbox')"
      /></footer>
    </section>
  </section>
