...
describe("青鸾 Console 17 屏结构保真", () => {
  ...
  it("上行回复使用单行检索条和密表格", () => {
    ...
    expect(workspace).toMatch(/\.reply-filter-bar,\s*\.blacklist-filter-bar,\s*\.sensitive-filter-bar\s*\{[^}]*display:\s*flex/s)
    expect(workspace).toMatch(/\.reply-seg,\s*\.blacklist-seg,\s*\.sensitive-policy-seg\s*\{[^}]*border-radius:\s*7px/s)
  })
