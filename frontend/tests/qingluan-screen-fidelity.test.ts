...
describe("青鸾 Console 17 屏结构保真", () => {
  ...
  it("上行回复使用单行检索条和密表格", () => {
    ...
    expect(workspace).toMatch(/\.reply-filter-bar,\s*\.blacklist-filter-bar,\s*\.sensitive-filter-bar,\s*\.user-filter-bar,\s*\.config-filter-bar\s*\{[^}]*display:\s*flex/s)
    expect(workspace).toMatch(/\.reply-seg,\s*\.blacklist-seg,\s*\.sensitive-policy-seg,\s*\.user-seg,\s*\.config-seg\s*\{[^}]*border-radius:\s*7px/s)
  })
  ...
  it("用户与角色使用单行检索条和身份台账抽屉", () => {
    ...
    expect(workspace).toMatch(/\.reply-filter-bar,\s*\.blacklist-filter-bar,\s*\.sensitive-filter-bar,\s*\.user-filter-bar,\s*\.config-filter-bar\s*\{[^}]*display:\s*flex/s)
    expect(workspace).toMatch(/\.reply-seg,\s*\.blacklist-seg,\s*\.sensitive-policy-seg,\s*\.user-seg,\s*\.config-seg\s*\{[^}]*border-radius:\s*7px/s)
    expect(workspace).toMatch(/\.user-rules,\s*\.config-rules\s*\{[^}]*display:\s*flex/s)
