...
26. **认证方式与浏览器会话（v1.6.4）**：Web access JWT 走 `Authorization: Bearer` 请求头，前端请求层统一注入，**不得**改走 Cookie。refresh JWT 走受限路径的 HttpOnly Cookie：生产必须 `Secure`、`SameSite=Lax`、`path` 限定为 `/api/v1/web/auth`，并与登录时的 `_tab_id`（请求体 `tab_id`）绑定；刷新与注销必须做规范化同源校验（Origin/Referer 对 canonical origin）。这是 Cookie 会话所需的 CSRF 补偿，**不是**额外的 cookie CSRF token。普通 access 仅存当前标签页 `sessionStorage`；refresh 不得写入 Web Storage。注销、401、超时和强制下线必须清理 access 并通过不含凭据的跨标签页信号同步，同时由服务端清除 refresh Cookie。首次改密、step-up 等高风险短期令牌只可存在于组件局部易失内存，禁止写入 Pinia、sessionStorage、localStorage、IndexedDB、URL 或日志，组件销毁时必须清空
