...
官方合同未提供分页、游标或单次最大条数。平台因此施加有界恢复合同：自动解析上限 4 MiB；4–64 MiB 的完整响应保全为 `complete_too_large` 仅供人工恢复；超过 64 MiB 或 spill 配额时完整性终止，截断密文标记 `truncated`；畸形 consume-on-read `Content-Length` 仍须有界保全正文并标记 `protocol_invalid`。截断与协议异常 raw 不得当作正常可重放。Mock 与定向测试不得连接真实厂商。
