...
- 决策：保持 API 固定两个 Uvicorn worker、30 RPS 持续 60 秒和受理 P95 `<300ms` 的
  NFR-01 口径不变，只把 Compose 中 API 服务的 CPU 上限从后端公共 `1.5` 核覆盖为
  `2` 核；realtime/bulk/callback、beat 与 outbox 等其他后端服务继续继承 `1.5` 核。
- 原因：同一运行时代码在 Hosted runner 的完整性能档连续两次仅 PERF-01 失败，P95 分别
  为 343ms 和 354ms，而安全、API E2E、迁移与其他组件门禁均成功。继续重跑会把容量门禁
  变成概率门禁，放宽阈值或缩短负载又会削弱 PRD；API 两个进程共同受限于 1.5 核是当前
  最窄的可控容量瓶颈。
- 影响：API 容器最多可使用两核，但进程数、数据库连接预算、领导者选举、安全边界和其他
  容器资源上限不变。该变更必须先以人工完整 CI 的默认性能档验证，不以 PR 选择性 G2、
  短参数或单元测试替代。
