...
const stepUpAction = ref<"activate" | "resume_critical" | null>(null)
...
const statusPresentation = computed(() => {
  ...
  if (status.value.mode === "setup_required") {
    return { title: "待完成设置", detail: "请先安装正式凭据并登记测试号码", tone: "warning" }
  }
...
async function load(): Promise<void> {
  ...
  try {
    ...
    if (!disposed) errorMessage.value = error instanceof Error ? error.message : "真实联调状态加载失败"
  } finally {
...
<template>
  <section v-loading="loading" class="vendor-test-console" aria-labelledby="vendor-test-heading">
    ...
    <el-dialog
      ...
      :title="stepUpAction === 'activate' ? '二次认证激活' : '二次认证恢复'"
      width="440px"
      destroy-on-close
      append-to-body
      @closed="clearStepUp"
    >
      ...
      <template #footer>
        <el-button :disabled="controlBusy" @click="stepUpVisible = false">继续检查</el-button>
        <el-button type="primary" :loading="controlBusy" @click="submitStepUp">
          {{ stepUpAction === 'activate' ? '验证并激活' : '验证并恢复' }}
        </el-button>
      </template>
    </el-dialog>
  </section>
