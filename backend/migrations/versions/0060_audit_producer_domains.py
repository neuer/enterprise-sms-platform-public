"""Split autonomous audit signing authority by producer domain."""

from __future__ import annotations

from alembic import op

revision = "0060_audit_producer_domains"
down_revision = "0059_authenticated_audit_alert"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.execute(
        "ALTER TABLE audit_context_signing_key "
        "DROP CONSTRAINT IF EXISTS audit_context_signing_key_key_kind_check"
    )
    # 扩展式迁移只重命名旧 key；新触发器永不读取该 inert legacy kind。
    op.execute(
        "UPDATE audit_context_signing_key "
        "SET key_kind='legacy-system-disabled',updated_at=now() "
        "WHERE key_kind='system'"
    )
    op.execute(
        """
        ALTER TABLE audit_context_signing_key
        ADD CONSTRAINT audit_context_signing_key_key_kind_check
        CHECK (key_kind IN (
...
def downgrade() -> None:
    # fail closed：不恢复共享 system key 或跨生产者域权限。
    pass
