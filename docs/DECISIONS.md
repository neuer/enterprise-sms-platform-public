...
- 影响：性能脚本运行环境继续使用后端锁定的 HTTPX 依赖；连接池由单个脚本进程共享并在
  退出时关闭。契约测试以真实本地 HTTP/1.1 server 证明连续请求复用同一连接；仍必须先
  以人工完整 CI 默认性能档验证，短参数或 PR 选择性 G2 不构成交付证据。
