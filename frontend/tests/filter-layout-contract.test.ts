...
describe("全站筛选布局契约", () => {
  it("filter-grid 十二列栅格已随审计日志页改造退役", () => {
    expect(css).not.toContain(".filter-grid")
    expect(css).not.toContain(".filter-span-")
    for (const view of [
      messageView,
      replyView,
      auditView,
      ...
      expect(view).not.toContain("filter-span-")
    }
  })
  ...
  it("审计日志使用方案 A 单行检索条、常驻规则条与载荷差异抽屉", () => {
    expect(auditView).toContain("audit-filter-bar")
    expect(auditView).toContain("audit-fld")
    expect(auditView).toContain("audit-keyword")
    expect(auditView).toContain("audit-dates")
    expect(auditView).toContain("audit-more-trigger")
    ...
    expect(auditView).not.toContain("<el-card")
    ...
    expect(css).toMatch(/\.batch-more-trigger,\s*\.audit-more-trigger\s*\{/s)
  })
  ...
  it("回调任务使用方案 A 单行检索条、常驻规则条与详情抽屉", () => {
    ...
    expect(callbackView).toContain("callback-confirm-dialog")
    expect(callbackView).toContain("callback-confirm-audit")
    ...
    expect(callbackView).not.toContain("<el-segmented")
    expect(callbackView).not.toContain("<el-card")
  ...
  it("安全日报使用页级紧凑单行检索条、常驻规则条与结果面板", () => {
    ...
    expect(securityDailyView).toContain("security-daily-privacy")
    expect(securityDailyView).toContain("security-daily-rules")
    expect(securityDailyView).toContain("security-daily-results")
    expect(securityDailyView).toContain("security-daily-confirm-dialog")
    expect(securityDailyView).toContain("security-daily-confirm-audit")
    ...
    expect(securityDailyView).not.toContain("<el-segmented")
    expect(securityDailyView).not.toContain("<el-card")
    expect(securityDailyView).not.toContain("security-daily-filters")
    expect(securityDailyView).not.toContain("card-heading")
    // 查询/重置与筛选条件同处一行 form，说明细字在条外下一行
    expect(securityDailyView).toMatch(
    ...
    expect(css).not.toMatch(/\.callback-seg,\s*\.security-daily-seg/)
    expect(css).toMatch(
      /\.user-rules,\s*\.config-rules,\s*\.ops-rules,\s*\.callback-rules,\s*\.security-daily-rules,\s*\.audit-rules\s*\{[^}]*display:\s*flex/s,
    )
  })
  ...
  it("不再保留会覆盖共享栅格的旧筛选补丁", () => {
    ...
    expect(css).not.toMatch(/\.query-filter,\s*\.message-search,\s*\.audit-filter-card \.el-form\s*\{/s)
  })
})
