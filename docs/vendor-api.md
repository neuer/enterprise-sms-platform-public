...
> **平台同步约束**：已有厂商编号的模板和签名无论当前为 pending、approved 或 rejected，平台都会每 10 分钟继续查询，且允许三态双向变化。平台要求每次批量返回与请求 ID 一一对应；缺失、额外、重复或非法项均整批拒绝写回。仅 rejected 保存已打码且最长 256 字的 checkRemark，其他状态清空旧驳回原因。模板与签名的厂商编号在平台内必须一对一绑定；模板一旦取得厂商编号即不可原地修改、重新绑定或删除，被拒绝或撤销后应新建。生产非 Mock 发送必须同时满足 approved 与 1..2147483647 的规范十进制厂商编号；状态降级阻止后续新受理，已受理批次使用冻结内容、签名和原编号继续到终态。
...
| 1010 | IP校验未通过 | crit 告警（当前启用出口必须书面报备；未来备出口启用前独立验证） |
...
| (HTTP超时/网络异常) | 结果未知 | **chunk=uncertain，禁自动重发或自动换供应商，reconcile 比对修复** |

### 4.1 多供应商 `safe_to_failover`

仅当厂商以协议错误明确拒绝且合同保证未受理时，才允许为同一 chunk 选择另一已注册供应商。
当前标记为可切换的代码：`999/1000/1001/1002/1003/1005/1008/1009/1010/1012/5000/10000/10001/10002/10003/10004/10005/10007/10008`。
`429/5002/5003` 只允许同一供应商退避；HTTP 超时、协议损坏、`5001` 和其他未知码一律 `uncertain` 或 failed，不得 failover。
第二生产供应商上线前，生产注册表仍只有 `zhihui`。
