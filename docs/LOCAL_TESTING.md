...
脚本会创建缺失的开发 `.env` 和 25 件 mock secrets，使用 dev profile 构建全栈，等待 API/mock 健康并执行幂等 `seed-dev`。已有 secret 内容不会被覆盖或打印。默认端口为：
...
- `.env`、25 件本地测试 secrets、开发 API Key、数据库卷均是本机测试资产，已被 Git
