...
async def live_audit_principal(accept_runtime, monkeypatch, request):
    ...
    try:
        ...
        async with owner.begin() as connection:
            await connection.execute(text("DELETE FROM audit_log WHERE actor_account_id=:id"),
                                     {"id": principal.account_id})
            await connection.execute(text("DELETE FROM auth_identity WHERE id=:id"),
                                     {"id": principal.identity_id})
            await connection.execute(text("DELETE FROM user_account WHERE id=:id"),
                                     {"id": principal.account_id})
