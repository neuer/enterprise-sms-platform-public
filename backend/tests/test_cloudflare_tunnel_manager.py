...
def fixture(tmp_path: Path):
    ...
    platform_dotenv_path = tmp_path / "opt/sms-platform/.env"
    platform_dotenv_path.parent.mkdir(parents=True)
    platform_dotenv_path.write_text(
        "WEB_BIND_IP=127.0.0.1\n"
        "SMS_EXTERNAL_TLS_MODE=1\n"
        f"SMS_TRUSTED_PROXY_CIDRS={module.TRUSTED_PROXY_CIDRS}\n"
        "SMS_TRUSTED_PROXY_CONF=/usr/local/share/sms-platform/trusted-proxies.conf\n",
        encoding="utf-8",
    )
    platform_dotenv_path.chmod(0o600)
    trusted_proxy_config_path = (
        tmp_path / "usr/local/share/sms-platform/trusted-proxies.conf"
    )
    trusted_proxy_config_path.parent.mkdir(parents=True)
    trusted_proxy_config_path.write_bytes(module._expected_trusted_proxy_config())
    trusted_proxy_config_path.chmod(0o644)
    paths = {
    ...
    manager = module.CloudflareTunnelManager(
        ...
        platform_dotenv_path=platform_dotenv_path,
        trusted_proxy_config_path=trusted_proxy_config_path,
...
def test_named_tunnel_proxy_contract_matches_canonical_renderer() -> None:
    module = manager_module()
    renderer = importlib.import_module("render_trusted_proxy_conf")

    assert module._expected_trusted_proxy_config() == renderer.render(
        "1", module.TRUSTED_PROXY_CIDRS
    ).encode()
...
def test_start_fails_closed_when_trusted_proxy_dotenv_is_missing(
    tmp_path: Path,
) -> None:
    module, manager, runner, *_ = fixture(tmp_path)
    manager.install()
    manager.configure("sms.example.invalid")
    manager.install_token()
    manager.platform_dotenv_path.unlink()

    with pytest.raises(module.CloudflareTunnelManagerError, match="asset|proxy"):
        manager.start()

    assert runner.states["sms-platform-cloudflare-tunnel.service"] == "inactive"


def test_verify_rejects_rendered_trusted_proxy_drift(tmp_path: Path) -> None:
    module, manager, runner, *_ = fixture(tmp_path)
    manager.install()
    manager.configure("sms.example.invalid")
    manager.install_token()
    manager.start()
    manager.trusted_proxy_config_path.write_text(
    ...
    assert runner.states["sms-platform-cloudflare-tunnel.service"] == "active"
