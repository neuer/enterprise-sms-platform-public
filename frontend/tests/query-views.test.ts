...
describe("批次与号码查询", () => {
  ...
  it("号码列表保持掩码并允许 approver 在徽标条授权查看", async () => {
    ...
    expect(wrapper.get("form.message-search").classes()).toContain("message-filter-bar")
    expect(wrapper.get("form.message-search").classes()).not.toContain("filter-grid")
    ...
    expect(wrapper.find(".status-tag--delivered").exists()).toBe(true)
    expect(wrapper.text()).toContain("共 1 条 · 每页 20")
    expect(wrapper.text()).not.toContain("13800138000")
    expect(wrapper.text()).not.toContain("在途")
    ...
    await wrapper.get("[data-testid='message-phone-decrypt']").trigger("click")
    await flushPromises()
    expect(wrapper.text()).toContain("13800138000")
    expect(wrapper.text()).toContain("已授权查看")
    expect(toast).toHaveBeenCalledWith("已解密 · 本次授权查看已记入审计")
  ...
  it("时间线展示号码徽标并将用户回复标为回声", async () => {
    ...
    await wrapper.find('input[placeholder="输入 11 位手机号"]').setValue("13800138000")
    await wrapper.get("[data-testid='message-view-timeline']").trigger("click")
    await flushPromises()
    ...
    expect(wrapper.text()).not.toContain("已触发退订")
    vi.unstubAllGlobals()
  })
