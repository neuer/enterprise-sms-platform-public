...
PLATFORM_DOTENV_PATH = Path("/opt/sms-platform/.env")
TRUSTED_PROXY_CONFIG_PATH = Path(
    "/usr/local/share/sms-platform/trusted-proxies.conf"
)
PLATFORM_SERVICE = "sms-platform.service"
MAX_FILE_BYTES = 8192
DOTENV_MAX_FILE_BYTES = 65536
START_TIMEOUT_SECONDS = 30.0
TRUSTED_PROXY_HOSTS = ("127.0.0.1/32", "172.31.250.1/32")
TRUSTED_PROXY_CIDRS = ",".join(TRUSTED_PROXY_HOSTS)
...
def _expected_trusted_proxy_config() -> bytes:
    hosts = "\n".join(f"    {host} 1;" for host in TRUSTED_PROXY_HOSTS)
    real_ip_hosts = "\n".join(
        f"set_real_ip_from {host};" for host in TRUSTED_PROXY_HOSTS
    )
    return (
...
def _validate_trusted_proxy_dotenv(raw: bytes) -> None:
    try:
        text = raw.decode("utf-8")
    except UnicodeError as exc:
        raise CloudflareTunnelManagerError(
            "cloudflare tunnel trusted proxy dotenv is invalid"
        ) from exc
    ...
    required = {
        ...
        "SMS_TRUSTED_PROXY_CIDRS": TRUSTED_PROXY_CIDRS,
        "SMS_TRUSTED_PROXY_CONF": str(TRUSTED_PROXY_CONFIG_PATH),
    }
    values: dict[str, str] = {}
    ...
    if values != required:
        raise CloudflareTunnelManagerError(
            "cloudflare tunnel trusted proxy contract is unavailable"
        )
...
class CloudflareTunnelManager:
    ...
    def __init__(
        ...
        *,
        ...
        platform_dotenv_path: Path = PLATFORM_DOTENV_PATH,
        trusted_proxy_config_path: Path = TRUSTED_PROXY_CONFIG_PATH,
        expected_uid: int = 0,
        ...
        self.platform_dotenv_path = platform_dotenv_path
        self.trusted_proxy_config_path = trusted_proxy_config_path
    ...
    def _validate_trusted_proxy_contract(self) -> None:
        dotenv = _read_bounded_file(
            self.platform_dotenv_path,
            expected_uid=self.expected_uid,
            expected_mode=0o600,
            max_bytes=DOTENV_MAX_FILE_BYTES,
        )
        _validate_trusted_proxy_dotenv(dotenv)
        rendered = _read_bounded_file(
            self.trusted_proxy_config_path,
            expected_uid=self.expected_uid,
            expected_mode=0o644,
        )
        if rendered != _expected_trusted_proxy_config():
            raise CloudflareTunnelManagerError(
                "cloudflare tunnel trusted proxy configuration has drifted"
            )
    ...
    def start(self) -> ManagerResult:
        self._validate_host_assets()
        self._require_installed_unit()
        self._validate_trusted_proxy_contract()
