...
case "$domain" in
  ...
  auth)
    ...
    command_rules='+ping +get +set +del +exists +incr +expire +pexpire +ttl +pttl +hget +hset +hsetnx +hdel +hmget +hgetall +type +persist +zadd +zrem +zrange +zrangebyscore +zcard +zscore +scan +time +eval +evalsha'
    ;;
  control)
    maxmemory='192mb'
    ...
    command_rules='+ping +get +set +del +mget +exists +incr +incrby +expire +pexpire +pexpireat +ttl +pttl +hget +hset +hincrby +hdel +hgetall +hlen +hmget +type +sadd +srem +smembers +smismember +zadd +zcard +zrange +zrem +zremrangebyscore +time +eval +evalsha +script|load +multi +exec +discard +watch +unwatch +scan'
    ;;
esac
