...
- Web 登录必须**显式选择认证源**并提交 `provider_code`；服务端只调用所选 Provider，**禁止自动回退**到其他认证源。所有 Provider 共享可恢复的账号失败阈值（第 5 次错误起返回 `ACCOUNT_LOCKED`，标记 15min）、IP 限流（5min 内失败≥20 封 15min）和统一错误话术；账号失败标记不得在密码校验前阻断，正确凭据只有在 Provider 校验及权威账号/身份绑定成功后才能清除标记，避免攻击者仅凭登录名定向锁死账号。访问 JWT 固定 15 分钟，refresh JWT 最长 7 天且每次使用后由 Redis Lua 原子单次轮换；5 秒有界且不可延长的 grace 内，同一旧 token 的合法并发或丢响应重试不得吊销 family，超出窗口的重放仍吊销该 refresh family。成功登录若请求携带既有 refresh Cookie，须先吊销该 Cookie family 再签发新 family，避免在途旧 refresh 的 Set-Cookie 覆盖新会话；登录失败或仅返回首次改密令牌时不吊销。
...
- AD refresh family 自最后一次完整密码认证起默认最多 480 分钟（可配置 15–10080 分钟）；多次 refresh 不得延长该起点。首次签发、Access 验证与 Refresh 验证必须绑定同一 `AuthSessionPolicy` 快照（`revision` 与期限），禁止回退内置 480 分钟或把缺失 revision 写成 `1`。策略延长只影响下一次完整 AD 登录，不得延长已签发会话的 Deadline。达到边界时必须原子吊销目标 family 并返回 `AUTH_REAUTH_REQUIRED`/401；显式 Refresh 入口同时删除 refresh Cookie，普通 Bearer 请求错误仅拒绝本请求、不得修改可能已属于新登录的 Cookie，要求重新输入密码；local family 仍保持现有最长 7 天。策略读取或吊销状态不可确认时返回 503 并 fail closed。
- 平台使用**全局不区分大小写登录名空间**，用户名规范化后唯一，归属按**先到先得**确定。本地账号创建时不探测 AD；后续真实 AD 登录若与既有本地身份冲突，则拒绝并审计 `ACCOUNT_SOURCE_CONFLICT`，由管理员线下处理。
- 管理员高风险写入在短数据库事务内复核原账号、身份、认证源、角色和安全版本；与权限撤销共用事务锁，撤销先提交时写入失败，已消费的二次认证令牌不退还。
- 平台**不开放自助注册**，本地账号仅由管理员维护；管理员可创建、启停、重置临时密码、设置角色与强制下线，但不得重命名或硬删除账号。禁止停用自己，且不得停用或降级最后一个有效管理员。有效管理员须账号、身份与认证源均启用；AD 身份即使覆盖为管理员，也须命中且仅命中一个非空部门（同部门多个组允许）。账号、认证源与映射变更在同一管理员保护事务锁下按变更后的状态验证，失败整笔回滚。
...
- AD 默认禁用；管理员登录后在**系统配置页**维护非敏感草稿，依次保存、测试当前版本并激活。Bind 密码仍仅来自 Docker secret，CA 仅来自受控文件。AD 角色默认由组映射计算，允许管理员单人覆盖或恢复跟随。映射保存仅写入实际差异，无变化不使会话失效；一次逻辑替换按 Provider 在同一数据库事务内最多推进每个关联账号的安全版本一次。直接 SQL 映射变更仍由数据库触发会话失效。保存必须携带读取时的映射 revision；事务锁内校验不符返回 STATE_CONFLICT，保留客户端草稿，由管理员重新加载并合并变更。
...
- 日期范围明细导出使用次日上海零点作为 `end_exclusive`（不包含终点），包含所选结束日的全部小数秒；精确时间 `end` 继续包含终点，两者不能同时提交。
- 导出任务对象授权：外部仅使用不可枚举 UUID；admin 可访问已解析任务，approver 仅本人或同部门，operator/viewer 仅本人创建且固化部门非空、等于当前部门的掩码任务；调岗或降权后的新会话不继承历史跨部门/全局文件权限。无主报告仅当前 admin 可访问，历史任务缺少或具有未知 dataset 时失败关闭。状态与下载执行同一授权；创建人同时固化 `account_id`/`identity_id`，历史用户名不得反查猜测主体，无法证明时对所有角色 fail closed。明文下载需绑定稳定账号、当前 JWT 会话、IP 与具体任务的五分钟单次二次认证令牌，并写无手机号的事务审计
...
- 用户名与密码规则必须常驻显示；停用账号和密码重置须显式二次确认；保护当前管理员与最后一个有效管理员。本地后备管理员必须已完成首次改密且具备本地凭据；待改密账号不能作为交接保障。重置密码同样在事务内保护该条件。
- **管理员高风险重认证**：创建本地 admin、所有角色/覆盖策略和账号启停修改、密码重置、AD 草稿保存/启停/角色映射替换，必须重新验证当前操作者当前 Provider 的密码（无回退）。确认弹窗只说明后果，不能替代重新认证；本期不实施 MFA。非 admin 本地账号创建与强制下线沿用现有管理员授权。
- `POST /api/v1/web/admin/step-up` 对服务器规范化的非凭据参数签发 300 秒单次令牌，绑定稳定账号/身份、Provider、安全版本、access jti、可信 IP、操作和目标；创建绑定规范化用户名、显示名、部门、角色，密码重置绑定目标与 reset 操作，密码值不进入授权事实或审计。认证源启停绑定草稿版本并在数据库写入时再次核对。
- 最终请求携带 `X-Admin-Step-Up`，原子消费后仍校验当前 access JWT 与管理员权限；消费/审计/权威状态失败即拒绝，失败不恢复令牌。Redis 仅保存随机令牌的摘要键及无凭据绑定事实。重认证失败、签发、拒绝与消费均记录稳定审计，不记录密码、令牌或操作载荷。密码与令牌只存组件易失内存，关闭、会话清理和销毁时清空；迟到认证结果不得触发业务操作。
#### FR-19 系统参数与认证源
