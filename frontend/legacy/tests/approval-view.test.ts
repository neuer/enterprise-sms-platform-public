...
describe("审批中心", () => {
  it("本人提交的待审批单不显示操作按钮", async () => {
    ...
    expect(wrapper.find(".approval-mobile-list").exists()).toBe(true)
    expect(wrapper.get("[data-testid='mobile-approval-detail-1']").text()).toContain("查看详情")
    expect(wrapper.find("[data-testid='mobile-approval-actions-1']").exists()).toBe(false)
    expect(wrapper.get("[data-testid='mobile-approval-actions-2']").text()).toContain("通过")
    ...
    await wrapper.get("[data-testid='mobile-approval-detail-1']").trigger("click")
