...
function search(): void {
  filters.start = timeRange.value?.[0] ? toApiDateTime(timeRange.value[0]) : ""
  filters.end = timeRange.value?.[1] ? toApiDateTime(timeRange.value[1]) : ""
  filters.page = 1
  void load()
}
...
<template>
  <section class="page-heading audit-heading">
    <div
      ><p class="eyebrow">IMMUTABLE LEDGER / 不可变账本</p><h1>审计日志</h1
      ><p
        >覆盖全部写操作与敏感读取；append-only，运行角色只有新增与查询权限。载荷受数据库 PII
        约束保护，手机号与逐号密文无法写入。</p
      ></div
    >
  ...
  <form class="audit-filter-bar" @submit.prevent="search">
    ...
    <div class="audit-fld">
      ...
      <el-input
        ...
        class="audit-object-type"
        data-testid="audit-object-type"
        clearable
        placeholder="如 sys_config"
      />
    </div>
    <div class="audit-fld">
      <span>对象 ID</span>
      <el-input
        ...
        class="audit-object-id"
        data-testid="audit-object-id"
        clearable
        placeholder="批次号 / 配置 key"
      />
    </div>
    <div class="audit-fld">
      <span>时间范围</span>
    ...
    <div class="audit-fld">
      ...
      <el-popover v-model:visible="moreOpen" placement="bottom-end" :width="320" trigger="click">
        <template #reference>
          <button
            type="button"
            class="audit-more-trigger"
            :class="{ 'is-more-active': moreActive }"
            data-testid="audit-more-filters"
            >更多筛选 ▾<b v-if="moreActiveCount" class="audit-more-count">{{ moreActiveCount }}</b></button
          >
        </template>
        <div class="audit-more">
          <label>稳定账号 ID</label>
          <el-input
            v-model="filters.actorAccountId"
            data-testid="audit-account-id"
            clearable
            placeholder="account_id 精确匹配"
          />
          <label>关联 ID</label>
          <el-input
            v-model="filters.correlationId"
            data-testid="audit-correlation-id"
            clearable
            placeholder="request ID 精确匹配"
          />
        </div>
      </el-popover>
    </div>
  ...
  <aside class="audit-rules" aria-label="不可变账本与 PII 边界">
    <div
      ><span>不可变账本</span
      ><p
        >审计事件 append-only：七个运行角色均无 UPDATE/DELETE/TRUNCATE 权限；保留 36 个月，到期清理由 DBA
        变更单驱动。</p
      ></div
    >
    <div
      ><span>PII 边界</span
      ><p
        >载荷受数据库约束保护，手机号、逐号密文与 HMAC 列表无法写入
        audit_log；载荷只记数量与引用，本页与详情均为只读。</p
      ></div
    >
  </aside>
    ...
    <template v-if="items.length || loading">
      <el-table v-loading="loading" :data="items" row-key="id" class="audit-table" @row-click="detail"
        ><el-table-column label="稳定主体" min-width="150"
          ><template #default="{ row }">{{
            row.actor_account_id
              ? `账号 #${row.actor_account_id}`
              : row.actor_app_id
                ? `应用 #${row.actor_app_id}`
                : "历史未知"
          }}</template></el-table-column
        ><el-table-column prop="actor" label="操作人快照" min-width="120" /><el-table-column
          prop="action"
          label="动作"
          min-width="170"
          ><template #default="{ row }"
        ...
        ><el-table-column label="对象" min-width="190"
          ><template #default="{ row }"
            >{{ row.object_type || "—" }} · {{ row.object_id || "—" }}</template
          ></el-table-column
        ><el-table-column prop="ip" label="IP" width="135" /><el-table-column label="时间" width="180"
      ...
      <div class="audit-mobile-list"
        ><article v-for="item in items" :key="item.id"
          ...
          ><strong>{{ item.actor }} · {{ item.role || "—" }}</strong
          ><p>{{ item.object_type || "—" }} / {{ item.object_id || "—" }}</p
          ><el-button link type="primary" @click="detail(item)">详情</el-button></article
        ></div
      >
    </template>
  ...
  <el-drawer v-model="drawer" title="审计事件详情" size="min(560px, 92vw)" :teleported="false" class="audit-drawer">
    <template v-if="selected"
      ><el-descriptions :column="1" border
        ><el-descriptions-item label="事件">#{{ selected.id }} · {{ selected.action }}</el-descriptions-item
        ><el-descriptions-item label="关联 ID"
          ><div class="audit-correlation"
            ><code>{{ selected.correlation_id }}</code
            ><el-button link type="primary" data-testid="audit-copy-correlation" @click="copyCorrelation"
              >复制</el-button
            ><el-button link type="primary" data-testid="audit-trace-correlation" @click="traceCorrelation"
              >同链路事件</el-button
            ></div
          ></el-descriptions-item
        ><el-descriptions-item label="稳定主体"
          >{{ selected.actor_subject_kind }} / account={{ selected.actor_account_id || "—" }} / identity={{
            selected.actor_identity_id || "—"
          }}
          / app={{ selected.actor_app_id || "—" }}</el-descriptions-item
        ><el-descriptions-item label="操作人快照"
          >{{ selected.actor }} / {{ selected.role || "—" }}</el-descriptions-item
        ><el-descriptions-item label="来源 IP">{{ selected.ip || "—" }}</el-descriptions-item
        ><el-descriptions-item label="对象"
          >{{ selected.object_type || "—" }} / {{ selected.object_id || "—" }}</el-descriptions-item
        ><el-descriptions-item label="时间">{{
          formatDateTime(selected.created_at)
        }}</el-descriptions-item></el-descriptions
      ><section class="audit-diff" aria-label="载荷前后差异"
        ><header class="audit-diff-head"><span>字段</span><span>BEFORE</span><span>AFTER</span></header
        ><template v-if="selectedDiff.length"
          ><div v-for="row in selectedDiff" :key="row.key" class="audit-diff-row" :class="`is-${row.state}`"
            ><div class="audit-diff-key"
              ><code>{{ row.key }}</code
              ><em v-if="row.state !== 'same'">{{ DIFF_LABEL[row.state] }}</em></div
            ><span class="audit-diff-value audit-diff-value--before">{{ row.before }}</span
            ><span class="audit-diff-value audit-diff-value--after">{{ row.after }}</span></div
          ></template
        ><p v-else class="audit-diff-empty">该事件无 before/after 载荷记录</p></section
      ><el-alert
        title="载荷受数据库 PII 约束保护"
        type="success"
        :closable="false"
        description="手机号、逐号密文与 HMAC 列表无法写入 audit_log。"
    /></template>
  </el-drawer>
</template>
