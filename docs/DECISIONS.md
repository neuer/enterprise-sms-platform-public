...
- 决策：日常 `classify-nul` 与 `apply` 的禁止路径保持不变；只为“服务器 commit 是目标
  ...
  `rebaseline`。它只额外接受两个固定运行控制脚本和枚举的非运行态文件；两脚本如有变化
  必须完整成对出现，两者均未变化时不要求制造伪差异，并强制 API/Web 双镜像与 high-risk 路径。性能门禁脚本及其权威说明
  `docs/PERFORMANCE.md` 只在该一次性入口按非运行态证据枚举，日常 `apply` 仍拒绝二者。
- 证据：目标合并 commit 必须有 GitHub Actions 产生的精确 `ci-gate=success`。该 gate 只有
  在同树 PR head 的 `backend`、`frontend`、`security`、`g2` 成功证据均完成绑定后才会成功，
  因此合并 SHA 上按设计 skipped 的重复 jobs 不要求再次执行。
...
## D066 Hosted G2 的 API 受理 P95 上限调整为 350ms

- 决策：本决策只取代 D065 与 NFR-01 中 API 受理 P95 `<300ms` 的阈值部分，将权威 G2、
  手工性能验收和全日 10 万条验收统一调整为 `<350ms`。30 RPS、持续 60 秒、2:3:5 类别
  权重、两个 API worker、有界 keep-alive 连接池、HTTP 零失败、verify P95 `<2s` 和排空
  `480s` 均保持不变；等于 350ms 仍失败关闭。
- 证据：`add68bc232d58e309c8e50c64acc21248be0735f` 的最终 main 全量 CI 仅 PERF-01 失败，
  观测为 P50 43ms、P90 173ms、P95 300ms，严格 `<300ms` 在边界相等时拒绝；同树 PR G2
  已成功。该失败未伴随 HTTP 错误、功能门禁失败或其他性能口径变化。
- 影响：API 受理预算放宽 50ms（约 16.7%），为 Hosted runner 的尾延迟保留有限余量；
  PRD、脚本、契约测试和两份性能手册在同一提交中同步，后续不得通过环境变量继续扩大。

## D067 Hosted G2 的 API 受理 P95 上限调整为 450ms

- 决策：本决策取代 D066 的 350ms API 受理门槛，将 NFR-01、权威 G2、手工性能验收和
  全日 10 万条验收统一为严格 `<450ms`。30 RPS、持续 60 秒、2:3:5 类别权重、HTTP
  零失败、verify P95 `<2s`、排空 `480s`、两个 API worker 和有界连接池均保持不变。
- 证据：采用 D066 的 `149d925ff721b8b61630c69c7fa33866fbfd1176` 最终 main 全量
  CI 仍仅 PERF-01 失败，观测 P50 50ms、P90 284ms、P95 389ms、P99 603ms；backend、
  frontend、security 及其余 G2 阶段均成功。450ms 比该次 P95 高 61ms，避免用 400ms
  形成不足 3% 的新边界余量。
...
## D069 API 受理 P95 门槛放宽为 2000ms

- 决策：本决策取代 D067 的 450ms API 受理门槛，将 NFR-01、权威 G2、手工性能验收和
  全日 10 万条验收统一为严格 `<2000ms`。30 RPS、持续 60 秒、2:3:5 类别权重、HTTP
  零失败、verify P95 `<2s`、排空 `480s`、两个 API worker 和有界连接池均保持不变。
- 证据：`bfdd8a33c410291f024cb5b5ab58932776976570` 的精确 main full CI 仅 PERF-01 失败，
  观测 P50 52ms、P90 458ms、P95 686ms、P99 1252ms，六个窗口 P95 最高为 1303ms；
  backend、frontend、security 及其余已执行门禁均成功。1000ms 仍低于已观测窗口尾延迟，
  因此不能稳定达到本次明确的跳过性能阻断目标。
...
## D087 Import/Export 共用目录持久化合同与导出孤儿对账
...
## D097 性能压测退出全部日常 CI/G2 门禁

- 决策：本决策取代 D059 的“每日、人工或生产候选继续执行性能门禁”。PR、push、每日定时、
  人工 CI、主干验真和 `scripts/verify_all.sh` 均不再执行性能压测，也不再生成或消费
  `performance` 分类输出。G2 删除原性能阶段，后续前端与发布控制恢复阶段连续编号。
- 专项边界：`scripts/perf_smoke.py`、`scripts/locustfile.py` 与 `docs/PERFORMANCE.md` 保留，
  只允许测试负责人在隔离测试环境专项执行；正式结果必须绑定 commit、镜像和环境并独立归档，
  不得以 `ci-gate` 绿色状态冒充性能或容量证据。日常 CI 可继续对压测工具做静态检查和单元测试，
  但不得产生压测负载。
...
## D099 登录容量保留与账号失败状态可恢复

- 决策：本决策修订 D098 的账号锁定语义。用户名失败阈值仍由 Redis Lua 原子累计，第 5 次
  错误凭据起建立 15 分钟 UUID 标记、返回 `ACCOUNT_LOCKED` 并写最小审计，但该标记不得在
  Provider 密码校验前拒绝请求。只有正确凭据通过所选 Provider，且稳定账号、身份、Provider、
  登录名和安全版本完成权威绑定后，才可清除账号失败状态；未知账号、错误 Provider、停用主体
  和错误凭据均不得清除。来源 IP 的 prehash token bucket、5 分钟长窗与 20 次封禁保持不变，
  因此同一来源在失败标记后继续快速尝试可能先返回 `RATE_LIMITED`，不再承诺第 5–19 次恒为 423。
- 容量：交互式本地登录使用专用 `auth_login_hash` 有界 Argon2 池；首次改密、日常改密、所有
  已认证 step-up 及管理员创建/重置密码继续使用独立 `auth_hash` 池。用途只由服务端可信调用链
  选择，任何请求字段或 Header 均不得选择执行池。两个池各 1 worker、最多 2 个在途任务，
  防止改密流量把正常登录直接背压为 503。
- 原因：D098 的 pre-auth 用户名硬锁允许攻击者只凭已知登录名制造 15 分钟定向拒绝服务；同时
  登录与密码变更共用 Argon2 池，使低权限已认证主体可以用并发改密挤占登录容量。正确凭据恢复
  与工作负载隔离关闭这两条路径，同时保留 IP 级资源防护、统一错误、失败审计和错误凭据阈值。
...
- 决策：创建 `hold_until=now()+60s` 的同一次转换必须返回
  `degraded/recovery_hold`，禁止保存 `open + future hold`。bootstrap 与
  `valid_until` 过期视为 previous=CLOSED。hold 期内 raw=OPEN 仍保持
  recovery_hold；raw=CLOSED 立即关闭并清空 hold。0105 先修复存量再加
  CHECK。
