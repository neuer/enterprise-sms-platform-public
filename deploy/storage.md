...
五个挂载点必须对应五个不同的 guest block device，文件系统只允许 `xfs` 或 `ext4`。
...
1. 创建四个数据盘挂载点，分别格式化为已批准的 `xfs` 或 `ext4`，再记录实际 UUID。
...
3. 逐个挂载并用 `findmnt --target <固定挂载点>` 核对 UUID、文件系统、`rw` 和设备号；
...
执行一次检查。预检失败不得用 `ExecStartPre=-...`、临时删除依赖或手工执行 Compose 绕过。
...
3. 按实际布局扩展 partition 或 LVM logical volume；不得照抄未知 `/dev/sdX`。
4. XFS 对固定挂载点执行 `xfs_growfs <mountpoint>`；ext4 对已核对的 block device 执行
