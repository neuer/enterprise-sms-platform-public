...
function goToQingluan(): void {
  window.location.assign(`/next${route.fullPath}`)
}
    ...
    <div v-else :class="['app-shell', { 'navigation-open': navigationOpen }]">
      <aside id="primary-navigation" class="sidebar" aria-label="应用导航">
        <div class="brand" data-testid="brand">
          <span class="brand-mark" aria-hidden="true">青</span>
          <div>
            <strong>青鸾</strong>
            <small>短信运营控制台</small>
          </div>
      ...
      <div class="workspace">
        <header class="topbar">
          ...
          <div class="operator">
            <button class="version-switch" type="button" @click="goToQingluan">体验青鸾版</button>
