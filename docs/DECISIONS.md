...
- 决策：仅为精确 `0053_idempotency_scope → 0061_vendor_binding_outbox` rebaseline 在迁移、
  ...
  `recover-rebaseline-verify`。入口要求不可变快照 commit、请求 target、实际 HEAD、实际
  migration head 完全一致，错误类型为 `invariant_failed`，且两条 pause 仍由原 update id
  持有；普通 apply、其他 blocked step 或其他迁移均不接受。
