...
| `legacy_data_hmac_pepper_v1` | 必须为 NULL | 独立只读 `api_key_legacy_hmac_pepper`，或旧 `data_hmac_key` **文件原文** 按 `HMAC-SHA256("sms-api-key-pepper-v1", raw_text)` 派生 |
...
2. **保留 legacy credential**：若存在 B 类（0085 之前用 `data_hmac_key` 原文派 pepper）活动 Key，把当时的 secret **文件原文** 挂到 `api_key_legacy_hmac_pepper`，并在 `api_key_unclassified_algorithms` 写入 `legacy_data_hmac_pepper_v1` 或 `legacy_sha256`。
