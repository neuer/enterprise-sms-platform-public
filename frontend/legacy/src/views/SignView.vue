...
async function submit(): Promise<void> {
  loading.value = true
  try {
    if (editing.value) await updateSign(editing.value.id, name.value)
    else await createSign(name.value)
...
<template>
  <section class="page-heading sign-heading">
    ...
    <el-button v-if="session.role === 'admin'" type="primary" @click="openEditor()">申请签名</el-button>
  </section>
    ...
    <el-table v-loading="loading" :data="items" row-key="id" class="sign-table">
      ...
      <el-table-column v-if="session.role === 'admin'" label="操作" width="190" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="row.vendor_state === 'rejected'" link @click="openEditor(row)">修改</el-button>
    ...
    <div v-loading="loading" class="sign-mobile-list">
      <article v-for="item in items" :key="item.id">
        ...
        <footer v-if="session.role === 'admin'">
          <el-button v-if="item.vendor_state === 'pending'" :data-testid="`mobile-sign-sync-${item.id}`" link type="primary" @click="sync(item)">同步</el-button>
