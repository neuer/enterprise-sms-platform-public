...
def test_weighted_cost_lua_uses_fixed_time_buckets() -> None:
    ...
    assert "marker_state ~= 'active_v2'" in WEIGHTED_WINDOW_LUA
...
class CostWindowRedis(LuaRedis):
    """真实 Lua 解释 v1 Hash + v2 环形槽；测试里预先放入已冻结的 active_v2 marker。"""

    def __init__(self, now_sec: int, *, seed_active: bool = True) -> None:
        super().__init__(now_sec)
        if seed_active:
            self.seed_hash(
                WRITER_CUTOVER_MARKER_KEY,
                {
...
def _limiter(redis: CostWindowRedis) -> ApplicationRateLimiter:
    return ApplicationRateLimiter(AsyncLuaRedis(redis), nonce=lambda: "n")
...
@pytest.mark.asyncio
async def test_static_v1_baseline_uses_max_after_old_writers_are_fenced() -> None:
    now = 1_778_000_300
    redis = CostWindowRedis(now)
...
@pytest.mark.asyncio
async def test_migration_marker_is_not_written_by_business_traffic() -> None:
    now = 1_778_000_500
    redis = CostWindowRedis(now)
    limiter = _limiter(redis)
    first = dict(redis.hashes[WRITER_CUTOVER_MARKER_KEY])
    await limiter.consume_send_cost(
        app_id=7,
        recipient_count=1,
        segment_count=1,
    ...
    assert redis.hashes[WRITER_CUTOVER_MARKER_KEY] == first
    assert first["state"] == "active_v2"
    assert WRITER_CUTOVER_MARKER_KEY not in redis.expires
