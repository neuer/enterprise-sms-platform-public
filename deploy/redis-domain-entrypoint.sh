...
case "$domain" in
  ...
  control)
    ...
    command_rules='+ping +get +set +del +mget +exists +incr +incrby +expire +pexpire +pexpireat +ttl +pttl +hget +hset +hdel +hgetall +hmget +sadd +srem +smembers +smismember +zadd +zcard +zrange +zrem +zremrangebyscore +eval +evalsha +script|load +multi +exec +discard +watch +unwatch +scan'
    ;;
esac
