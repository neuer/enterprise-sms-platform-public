...
# 与 raw_spill.capture_reservation_bytes 对齐：固定 64KiB 内部帧合同。
# 不得 import raw_spill（raw_spill → crypto → settings 会成环）。
# 公式：max_frames*(INTERNAL_FRAME_SIZE + 每帧信封) 的明文容量项已含在
# 64MiB 里；此处只列证明容量所需的帧信封、文件头、控制帧和目录元数据。
RAW_SPILL_RECOVERY_CAPTURE_BYTES = 64 * 1024 * 1024
RAW_SPILL_INTERNAL_FRAME_SIZE = 64 * 1024
RAW_SPILL_MAX_INTERNAL_FRAMES = (
    RAW_SPILL_RECOVERY_CAPTURE_BYTES // RAW_SPILL_INTERNAL_FRAME_SIZE
)
RAW_SPILL_DATA_FRAME_OVERHEAD_BYTES = 36
RAW_SPILL_STREAM_HEADER_BUDGET_BYTES = 256
RAW_SPILL_CONTROL_FRAME_COUNT = 2
RAW_SPILL_CONTROL_FRAME_BUDGET_BYTES = 256
RAW_SPILL_DIRECTORY_METADATA_BYTES = 768
RAW_SPILL_CAPTURE_OVERHEAD_BYTES = (
    RAW_SPILL_MAX_INTERNAL_FRAMES * RAW_SPILL_DATA_FRAME_OVERHEAD_BYTES
    + RAW_SPILL_STREAM_HEADER_BUDGET_BYTES
    + RAW_SPILL_CONTROL_FRAME_COUNT * RAW_SPILL_CONTROL_FRAME_BUDGET_BYTES
