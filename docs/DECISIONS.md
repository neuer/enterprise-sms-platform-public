...
-   影响：schema v1.6.60、0072、`raw_spill`/`zhihui`/`report_ingest`/`reply_ingest`/
  自动重放过滤、运维 raw 列表、`vendor-api.md` 与 OpenAPI。历史存量分类见 D080。

## D080 pre-0072 存量 raw 不得默认回填为 complete

- 决策：`capture_state` 新行仍 DEFAULT `complete`，与 schema.sql 一致。升级存量时先
  以 `unknown_legacy` 占位暂停自动重放，再按密文长度（SME2/legacy 信封开销）和
  历史 error 类型分类：普通完整 → `complete`；4–64 MiB 且无截断证据 →
  `complete_too_large`；截断证据或超过 64 MiB → `truncated`；信封无效、恰好卡在
  4 MiB/64 MiB 上限、或错误类型与长度冲突 → `unknown_legacy`。不解密正文。
  `unknown_legacy` 与 `truncated` 一样不得进入普通自动/运维重放，须人工盘点后
  再提升。已执行旧 0072（全部 complete）的库由 0073 按同一证据重分类。
...
- 影响：schema v1.6.61、0072/0073、`raw_capture_legacy`、自动/运维重放资格、
