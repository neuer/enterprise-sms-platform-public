...
describe("青鸾 Console 17 屏结构保真", () => {
  ...
  it("运维中心使用自定义页签条、单行检索条与常驻规则条", () => {
    ...
    expect(workspace).toMatch(/\.user-rules,\s*\.config-rules,\s*\.ops-rules,\s*\.callback-rules,\s*\.security-daily-rules\s*\{[^}]*display:\s*flex/s)
  })
  ...
  it("上行回复使用单行检索条和密表格", () => {
    ...
    expect(workspace).toMatch(/\.reply-filter-bar,\s*\.blacklist-filter-bar,\s*\.sensitive-filter-bar,\s*\.user-filter-bar,\s*\.config-filter-bar,\s*\.ops-filter-bar,\s*\.callback-filter-bar\s*\{[^}]*display:\s*flex/s)
