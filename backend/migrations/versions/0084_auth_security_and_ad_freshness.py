"""登录安全状态审计与 AD 会话完整重新认证时限。"""

from __future__ import annotations

from alembic import op

revision = "0084_auth_security_and_ad_freshness"
down_revision = "0083_password_change_token_lease"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.execute(
        ...
        VALUES(
          'ad_session_max_age_minutes','480','int',
          'AD会话完整重新认证时限(分钟,15-10080)'
        )
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS idx_audit_auth_security_transition
...
def downgrade() -> None:
    # 历史锁定/封禁审计必须保留；回退仅关闭旧版本不会调用的新增写入能力。
    op.execute("DROP INDEX IF EXISTS idx_audit_auth_security_transition")
    op.execute(
        r"""
        DO $migration$
        DECLARE
        ...
        BEGIN
          SELECT pg_get_functiondef(
            'enforce_live_audit_principal()'::regprocedure
          ) INTO function_sql;
          IF position('auth_account_locked' IN function_sql) > 0 THEN
            updated_sql := regexp_replace(
              function_sql,
              $pattern$NEW\.action[[:space:]]+IN[[:space:]]*\([[:space:]]*'account_source_conflict'[[:space:]]*,[[:space:]]*'auth_account_locked'[[:space:]]*,[[:space:]]*'auth_ip_banned'[[:space:]]*\)$pattern$,
              $old$NEW.action='account_source_conflict'$old$
            );
            IF updated_sql IS NOT DISTINCT FROM function_sql THEN
              RAISE EXCEPTION
                'cannot restore auth system audit action allowlist safely';
            END IF;
            EXECUTE updated_sql;
          END IF;
        END;
