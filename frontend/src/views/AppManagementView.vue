...
<template>
  ...
  <section v-loading="loading" class="app-card-grid" aria-label="应用与密钥">
    <article v-for="row in items" :key="row.id" :class="['managed-app-card', { disabled: !row.status }]">
      <header><div><strong>{{ row.name }}</strong><small>#{{ row.id }} · {{ row.dept }}</small></div><el-tag :type="row.status ? 'success' : 'info'">{{ row.status ? '启用' : '停用' }}</el-tag></header>
      <div class="app-management-categories"><CategoryTag v-for="category in row.allowed_categories" :key="category" :category="category" /></div>
      <dl><div><dt>日配额</dt><dd>{{ row.daily_quota.toLocaleString() }}</dd></div><div><dt>每分钟限流</dt><dd>{{ row.rate_limit_per_min }}</dd></div><div><dt>默认签名</dt><dd>{{ row.default_sign || '未设置' }}</dd></div></dl>
      <div class="managed-key"><span>API KEY</span><code>sk-••••••••••</code><small>{{ keyGraceHours === null ? '旧 Key 宽限期暂不可用' : `旧 Key 宽限期 ${keyGraceHours} 小时` }}</small></div>
      <footer>
