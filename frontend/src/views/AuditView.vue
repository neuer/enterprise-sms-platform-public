...
<template>
  ...
  <el-drawer v-model="drawer" title="审计事件详情" size="min(520px, 100vw)" class="audit-drawer">
    <template v-if="selected"><el-descriptions :column="1" border><el-descriptions-item label="事件">#{{ selected.id }} · {{ selected.action }}</el-descriptions-item><el-descriptions-item label="关联 ID"><code>{{ selected.correlation_id }}</code></el-descriptions-item><el-descriptions-item label="稳定主体">{{ selected.actor_subject_kind }} / account={{ selected.actor_account_id || '—' }} / identity={{ selected.actor_identity_id || '—' }} / app={{ selected.actor_app_id || '—' }}</el-descriptions-item><el-descriptions-item label="操作人快照">{{ selected.actor }} / {{ selected.role || '—' }}</el-descriptions-item><el-descriptions-item label="来源 IP">{{ selected.ip || '—' }}</el-descriptions-item><el-descriptions-item label="对象">{{ selected.object_type || '—' }} / {{ selected.object_id || '—' }}</el-descriptions-item><el-descriptions-item label="时间">{{ time(selected.created_at) }}</el-descriptions-item></el-descriptions><section class="audit-payload"><div><span>BEFORE</span><pre>{{ json(selected.before_val) }}</pre></div><div><span>AFTER</span><pre>{{ json(selected.after_val) }}</pre></div></section><el-alert title="载荷受数据库 PII 约束保护" type="success" :closable="false" description="手机号、逐号密文与 HMAC 列表无法写入 audit_log。" /></template>
  </el-drawer>
</template>
