...
<template>
  ...
  <el-card shadow="never" class="approval-card">
    ...
    <el-table v-loading="loading" :data="items" row-key="id" @row-click="showDetail">
      ...
      <el-table-column label="预计计费" width="110" align="right">
        <template #default="{ row }">{{ formatSegments(row.estimated_segments) }}</template>
      </el-table-column>
      <el-table-column label="计划 / 规则" min-width="210">
        <template #default="{ row }"><div class="approval-decision-facts"><span>{{ formatSchedule(row.scheduled_at) }}</span><small>{{ triggerRule(row) }}</small></div></template>
      </el-table-column>
    ...
    <div v-loading="loading" class="approval-mobile-list">
      <article v-for="item in items" :key="item.id">
        ...
        <dl>
          ...
          <div><dt>单号计费</dt><dd>{{ formatSegments(item.segments) }}</dd></div>
